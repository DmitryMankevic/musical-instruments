...
static async getAllFavouritesItemsByUser(req, res) {
    ...
    const parsedUserId = Number(userId);   // Валидация ID 
    if (isNaN(parsedUserId) || parsedUserId <= 0) {
      return res.status(400).json(formatResponse(400, 'Некорректный ID пользователя'));
    }
    try {
      const favouritesItemsArr = await FavouriteService.getByUserId(parsedUserId);
      if (favouritesItemsArr.length === 0) {
        return res
          .status(400)
          .json(formatResponse(400, 'Товаров в избранном не обнаружено', []));
      }
  ...
  static async addItemInFavourites(req, res) {
    ...
    if (isNaN(parsedUserId) || isNaN(parsedItemId)) {
      return res.status(400).json(formatResponse(400, 'Некорректные ID пользователя или товара'));
    }
    try {
      const favourite = await FavouriteService.addItem(parsedUserId, parsedItemId);
      if (!favourite) {
        return res.status(400).json(formatResponse(400, 'Не удалось добавить в избранное'));
      }
      return res.status(201).json(formatResponse(201, 'Товар добавлен в избранное', favourite));
    } catch (error) {
  ...
  static async deleteItemFromFavourites(req, res) {
    ...
    const parsedUserId = Number(userId);   // Валидация ID пользователя и ID товара
    const parsedItemId = Number(itemId);
    if (isNaN(parsedUserId) || isNaN(parsedItemId) || parsedUserId <= 0 || parsedItemId <= 0) {
      return res.status(400).json(formatResponse(400, 'Некорректные ID пользователя или товара'));
    }
    try {
